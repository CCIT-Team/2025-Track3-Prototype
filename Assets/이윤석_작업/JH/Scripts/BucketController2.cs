...
public class BucketController2 : MonoBehaviour
{
    ...
    [Header("굴착기 컨트롤러 참조")]
    public ExcavatorController_public2 excavatorController;  // Inspector에서 직접 할당 또는 자동 검색

    private float _particleAccumulator;
    private Rigidbody _rb;
    private Collider _col;
    private TerrainDeformManager2 _deformer;
    private TerrainCollider _terrainCol;
    private Terrain _terrain;
    private BucketGrabberMulti2 _modeCtrl;
    [SerializeField]
    ...
    void Start()
    {
        ...
        _deformer = FindObjectOfType<TerrainDeformManager2>();
        if (_deformer == null)
        {
        ...
        // BucketGrabberMulti
        _modeCtrl = GetComponentInParent<BucketGrabberMulti2>();
        if (_modeCtrl == null)
        ...
        {
            excavatorController = FindObjectOfType<ExcavatorController_public2>();
            if (excavatorController == null)
    ...
    void FixedUpdate()
    {
        ...
        if (excavatorController != null && _modeCtrl != null)
        {
            ...
            BucketGrabberMulti2.Mode desired = BucketGrabberMulti2.Mode.Idle;
            if (angle >= dumpMinAngle && angle < dumpMaxAngle)
                desired = BucketGrabberMulti2.Mode.Dump;
            else if (angle >= digMinAngle && angle < digMaxAngle)
                desired = BucketGrabberMulti2.Mode.Dig;
            else if (angle >= idleMinAngle && angle <= idleMaxAngle)
                desired = BucketGrabberMulti2.Mode.Idle;
        ...
        // 2) Dig 모드가 아니면 종료
        if (_modeCtrl == null || _modeCtrl.CurrentMode != BucketGrabberMulti2.Mode.Dig || !isDigging)
            return;
        ...
        PixelTextureData[] textureDatas = _deformer.SelectTextures(bb.min, bb.max, toSpawn);
        ...
        for (int i = 0; i < toSpawn; i++)
        {
            ...
            if (go.TryGetComponent<SoilParticle2>(out var p))
            {
            ...
            {
                int index = TerrainPaintUtility.FindTerrainLayerIndex(_terrain, textureDatas[i].layer);
                if (index < 0)
                {
                    continue;
                }
